...
pruneCells branches =
  ...
  cellSlots
    (length branches)
    (mapMaybe cellSlot (zip slotRange (NonEmpty.toList (NonEmpty.transpose branches))))
 where
...
renderLine config@RenderConfig{slotDistance, forkChar} (SlotWidth width) aspects labelWidth =
 ...
 where
  lineString
    | elem Fork aspects = replicate (lineWidth - 1) ' ' ++ [forkChar]
...
renderColBlocks RenderConfig{candidateColors, selectionColor, slotNumberColor, cachedPeers} cols =
  first
    ...
    ( runCol
        ...
        slotNumberColor
        cachedPeers
        (traverse (traverse renderCol) cols)
    )
